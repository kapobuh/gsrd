...
define("EDOST_ID", 3744); //Идентификатор магазина (вписать вместо: 0)
define("EDOST_PSWD", "wSf0fIiElbRjWvQhoYfRV9vzbv04OE0Z"); //Пароль для доступа к серверу расчетов (вписать вместо: password)
//---------- ВНИМАНИЕ!!! ----------
...
var $kod = 1; //если 1, тогда перекодировать в UTF8 (win_utf8)
var $site_win = 0; //кодировка если 1, то надо win_utf8 (входящие параметры)
var $index, $curElem;
var $status = 0;
var $parser;
var $rz;
...
var $test=1; //включить тестовый вывод =1
...
//echo $rez;
	...
	function edost_calc($to_city, $free, $weight, $strah, $length, $width, $height, $zip='') {
	...
	function edost_calc_post() {
		...
		if (isset($_POST['edost_to_city'])) {

			if (isset($_POST['edost_kod'])) {
				$this -> kod=1;
			} else {
				$this -> kod=0;
			}
			...
			//////////////////////
			//$weight = 2;
			$to_city = $_POST['edost_to_city'];
			$zip = ($_POST['edost_zip'] ? $_POST['edost_zip'] : '');
			...
			if (!$weight) {
				$weight = 1; // по умолчанию ставим 1 кг
			}
			
			$strah = 0;
			$cartsum = (int)$_POST['cartsum'];
			if ($cartsum) {
				$strah = $cartsum;
			}			
			...
			$r = $this -> edost_calc($to_city, $_POST['free'], $weight, $strah, $length, $width, $height, $zip);
