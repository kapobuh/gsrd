...
class ControllerSaleOrder extends Controller {
	private $error = array();
	private $main_products = array ('261', '281', '260', '259', '262', '287', '286', '289', '288'); // Продукты - не наборы
    ...
    public function deletedOrders() {
		...
		$this->document->setTitle('Удаленные заказы');

		$this->load->model('sale/order');
        
        $status = 22; // id удаленных заказов в статусах заказов (в базе)
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validate()) {
			foreach ($this->request->post['selected'] as $order_id) {
				$this->model_sale_order->setDeletedOrders($order_id);
			}
	...
	protected function getList($status = false) {
		...
		//$data['invoice2'] = $this->url->link('sale/order/invoice2', '&token=' . $this->session->data['token'], true);
		...
		$data['export_excel'] = $this->url->link('tool/export/orders', 'token=' . $this->session->data['token'], true);
		$data['export_newsletter'] = $this->url->link('tool/export/newsletter', 'token=' . $this->session->data['token'], true);
		$data['export_coupons'] = $this->url->link('tool/export/orders', 'token=' . $this->session->data['token'], true);
		...
		foreach ($results as $result) {
			$data['orders'][] = array(
				...
				'invoice'          => $this->url->link('sale/order/invoice', 'token=' . $this->session->data['token'] . '&order_id=' . $result['order_id'] . $url, true),
				'invoice2'          => $this->url->link('sale/order/invoice2', 'token=' . $this->session->data['token'] . '&order_id=' . $result['order_id'] . $url, true)
        ...
        if ($status == '22') {
            $data['deleted_orders_page'] = true;
        } else {
            $data['deleted_orders_page'] = false;
        }
	...
	public function getForm() {
		...
		if (!empty($order_info)) {
			...
			$data['payment_firstname'] = $order_info['firstname']; ; //$order_info['payment_firstname'];
			$data['payment_lastname'] =	 $order_info['lastname']; //  $order_info['payment_lastname'];
			...
			$data['shipping_firstname'] = $order_info['firstname'];; //$order_info['shipping_firstname'];
			$data['shipping_lastname'] =$order_info['lastname'];  // $order_info['shipping_lastname'];
			...
			$data['shipping_val'] = $this->model_sale_order->getOrderShippingVal($this->request->get['order_id']);
	...
	public function info() {
		...
		if ($order_info) {
			...
			$data['invoice2'] = $this->url->link('sale/order/invoice2', 'token=' . $this->session->data['token'] . '&order_id=' . (int)$this->request->get['order_id'], true);
	...
	public function invoice($invtype = 1) {
		$this->load->language('sale/order');
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
				...
				foreach ($products as $product) {
					...
					// Если наборы, то выводим их состав в печать заказа
					if (!in_array($product['product_id'], $this->main_products)) {
						$nabors = array (
							//«Антигрипп»
							263 => '<ul><li>NFO Омега-3 Масло лосося;</li><li>NFO Омега-3 Жир печени акулы</li></ul>',
							
							// Семейный
							266 => '<ul><li>NFO Омега-3 жевательные капсулы с витамином D</li><li>NFO Омега-3 со вкусом лимона и NFO Омега-3 Форте</li><li>NFO Омега-3 Масло лосося </li></ul>',
							
							// Спорт+
							264 => '<ul><li>NFO Омега-3 со вкусом лимона 2x;</li><li>NFO Омега-3 Форте 2x</li></ul>',
							
							// Фитнес
							265 => '<ul><li>NFO Омега-3 со вкусом лимона;</li><li>NFO Омега-3 Форте</li></ul>',
							
							// Попурри 1
							267 => '<ul><li>NFO Омега-3 Масло лосося - x1</li><li>NFO Омега-3 со вкусом лимона - x1</li><li>NFO Омега-3 Жир печени акулы- x1</li></ul>',
							
							// Попурри 2
							268 => '<ul><li>NFO Омега-3 Форте - x1</li><li>NFO Омега-3 Масло лосося - x1</li><li>NFO Омега-3 Жир печени акулы - x1</li></ul>',
							
							// Попурри 3
							269 => '<ul><li>NFO Омега-3 Масло лосося - x1<br></li><li>NFO Омега-3 Жир печени акулы - x1</li><li>NFO Омега-3 Жевательные капсулы с витамином D - x1</li></ul>',
							
							// Попурри 4
							270 => '<ul><li>NFO Омега-3 Масло лосося - x1<br></li><li>NFO Омега-3 Масло лосося - x1</li><li>NFO Омега-3 Жир печени акулы - x1</li></ul>',
							
							// Попурри 5
							271 => '<ul><li>NFO Омега-3 Масло лосося - x1<br></li><li>NFO Омега-3 Жир печени акулы - x1</li><li>NFO Омега-3 Жир печени акулы - x1</li></ul>',
                            
                            // Осенний набор «Снова в школу»
							290 => '<ul><li>NFO Омега-3 Форте<br></li><li>NFO Омега-3 Жир печени трески</li><li>NFO Омега-3 Жевательные капсулы с витамином D</li></ul>',
                            
                            // Осенний набор «Защити иммунитет»
							291 => '<ul><li>NFO Омега-3 Форте<br></li><li>NFO Омега-3 Масло Криля</li><li>NFO Омега-3 Жир печени акулы</li></ul>'
							);
					} else {
						$nabors = false;
					}
				...
				if (($order_info['order_status_id'] == '17') || ($order_info['order_status_id'] == '21')) {
					$show_order_status = 1;
				} else {
					$show_order_status = 0;
				}
