...
class ControllerTestimonialTestimonial extends Controller {
	...
	protected function getForm() {
		...
		if (isset($this->request->post['response'])) {
			...
			$data['response'] = $review_info['response'];
		} else {
			$data['response'] = '';
		}
	...
	public function send() {
		...
		$review_info = $this->{$this->moduleModel}->getReview($review_id);
		
		$send = $this->{$this->moduleModel}->sendAnswer($review_info['email'], $this->request->post['review_text']);
		
		if ($send) {
			return 'Письмо успешно отправлено!';
		} else {
			return 'Письмо отправить не удалось.';
		}
