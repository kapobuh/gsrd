...
class ModelCustomerCustomer extends Model {
	public function addCustomer($data) {
		...
		if (isset($data['alert_email'])) {
			...
			$mail_data['logo'] = HTTP_CATALOG . 'image/catalog/image001.png';
			$mail_data['store_name'] = $this->config->get('config_name');
			$mail_data['store_url'] = $this->config->get('config_url');
		
			$mail_data['text_register_welcome'] = sprintf($this->language->get('text_register_welcome'), html_entity_decode($this->db->escape($data['firstname']), ENT_QUOTES, 'UTF-8'));
			$mail_data['text_register_registered'] = sprintf($this->language->get('text_register_registered'), html_entity_decode($this->config->get('config_name'), ENT_QUOTES, 'UTF-8'));
			$mail_data['text_register_dologin'] = $this->language->get('text_register_dologin');
			$mail_data['text_register_email'] = sprintf($this->language->get('text_register_email'), html_entity_decode($this->db->escape($data['email']), ENT_QUOTES, 'UTF-8'));
			$mail_data['text_register_password'] = sprintf($this->language->get('text_register_password'), html_entity_decode($this->db->escape($data['password']), ENT_QUOTES, 'UTF-8'));
			$mail_data['text_register_sub'] = sprintf($this->language->get('text_register_sub'), html_entity_decode($this->config->get('config_name'), ENT_QUOTES, 'UTF-8'));
			
				
				/*$headers  = 'MIME-Version: 1.0' . "\r\n";
                $headers .= 'Content-type: text/html; charset=utf-8' . "\r\n"; 
                $headers .= 'From: NFO <info@norwegianfishoil.ru>' . "\r\n";
                $html = $this->load->view('mail/register', $mail_data);
				
				$send_admin = mail ($this->db->escape($data['email']), html_entity_decode($subject, ENT_QUOTES, 'UTF-8'), $html, $headers);*/
			...
			$mail->setSubject(sprintf($this->language->get('text_register_subject'), html_entity_decode($this->config->get('config_name'), ENT_QUOTES, 'UTF-8')));
			$mail->setHtml($this->load->view('mail/register', $mail_data));
			$mail->send();
		}
