...
class ModelExtensionModuleTestimonial extends Model {
	...
	public function addReview($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "review SET author = '" . $this->db->escape($data['author']) . "', text = '" . $this->db->escape(strip_tags($data['text'])) . "', response = '" . $this->db->escape(strip_tags($data['response'])) . "', rating = '" . (int)$data['rating'] . "', status = '" . (int)$data['status'] . "', date_added = '" . $this->db->escape($data['date_added']) . "'");
	...
	public function editReview($review_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "review SET author = '" . $this->db->escape($data['author']) . "', text = '" . $this->db->escape(strip_tags($data['text'])) . "', response = '" . $this->db->escape(strip_tags($data['response'])) . "', rating = '" . (int)$data['rating'] . "', status = '" . (int)$data['status'] . "', date_added = '" . $this->db->escape($data['date_added']) . "', date_modified = NOW() WHERE review_id = '" . (int)$review_id  . "'");
		
		if (!empty($data['send_answer']) && ($this->db->escape($data['send_answer']) == 'on')) {
			$subject = 'Ответ на Ваш вопрос на сайте Norwegian Fish Oil';
			$headers  = 'MIME-Version: 1.0' . "\r\n";
			$headers .= 'Content-type: text/html; charset=utf-8' . "\r\n"; 
			$headers .= 'From: Norwegian Fish Oil <info@norwegianfishoil.ru>' . "\r\n";
			$html = '<p>Ответ на Ваш вопрос:</p><p>'.nl2br($data['response']).'</p>';
			$send = mail ($this->db->escape($data['email']), html_entity_decode($subject, ENT_QUOTES, 'UTF-8'), $html, $headers);
			//$send_admin2 = mail ("info@norwegianfishoil.ru", html_entity_decode($subject, ENT_QUOTES, 'UTF-8'), $html, $headers);
		}
	}
