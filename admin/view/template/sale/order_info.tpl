...
<div id="content">
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h3>Отправить SMS</h3>
					<h4 id="sms_balance">Баланс: </h4>
				</div>
				<div class="modal-body">
					<p id="sms_klient">Клиент: <?php echo $firstname; ?> <?php echo $lastname; ?></p>
					<?php if (!$fax) { ?>
					<p><small style="color:red;"><b><i class="fa fa-exclamation-circle" aria-hidden="true"> </i> Клиент не давал согласия получать SMS.</b></small></p>
					<?php } ?>
					<div class="row">
						
						
						 <div class="form-group">
							  <label class="col-sm-4 control-label text-right">Номер телефона</label>
							  <div class="col-sm-8">
								<input id="sms_phone" type="text" value="<?php echo $telephone; ?>" class="form-control"/>
							  </div>
							</div>
						...
						<div class="form-group">
						  <label class="col-sm-4 control-label text-right">Шаблон текста</label>
						  <div class="col-sm-8">
							<select id="sms_type" class="form-control">
								<option value="3">--Выберите</option>
							    <option value="0">ShopLogistic</option>
								<option value="1">Почта, СДЭК, Pony</option>
						...
						<br/>
							 <div class="form-group text-right">
							  <label class="col-sm-4 control-label" for="input-comment">Текст сообщения</label>
							  <div class="col-sm-8">
								<textarea name="sms_text" id="sms_text" rows="8" id="input-comment" class="form-control"></textarea>
							  </div>
							</div>
				...
				<div class="modal-footer">
					...
					<button id="sms_send_btn" disabled type="button" class="btn btn-primary"><i class="fa fa-check-circle" aria-hidden="true"></i> Отправить сообщение</button>
				</div>
			</div>
	  	</div>
	</div>
  <div class="page-header">
    <div class="container-fluid">
      <div class="pull-right"><button id="sms_btn" data-toggle="tooltip"  class="btn btn-primary" title="Отправить SMS" ><i class="fa fa-paper-plane" aria-hidden="true"></i></button> <a href="<?php echo $invoice; ?>" target="_blank" data-toggle="tooltip" title="<?php echo $button_invoice_print; ?>" class="btn btn-info"><i class="fa fa-print"></i></a> <a href="<?php echo $invoice2; ?>" target="_blank" data-toggle="tooltip" title="Печать без фона" class="btn btn-default"><i class="fa fa-print"></i></a> <a href="<?php echo $edit; ?>" data-toggle="tooltip" title="<?php echo $button_edit; ?>" class="btn btn-primary"><i class="fa fa-pencil"></i></a> <a href="<?php echo $cancel; ?>" data-toggle="tooltip" title="<?php echo $button_cancel; ?>" class="btn btn-default"><i class="fa fa-reply"></i></a></div>
  ...
  <script type="text/javascript"><!--
	...
	$(document).ready(function(){
		  ...
		  var sms_text = '';
		  /*sms_text = sms_text + "Ваш заказ № <?php echo $order_id; ?> передан в службу доставки.";
		  sms_text = sms_text + "\n";
		  sms_text = sms_text + "Сумма: <?php echo (int)$order_total; ?> р.";
		  sms_text = sms_text + "\n";
		  sms_text = sms_text + "Доставка 01.01.2017 с 12:00-15:00";*/
		  ...
		  $('#sms_type').change(function(){
			  ...
			  switch ($(this).val()) {
					  ...
					  case "0":	// SL
						  sms = sms + "Ваш заказ № <?php echo $order_id; ?> передан в службу доставки.";
						  sms = sms + "\n";
						  sms = sms + "Сумма: <?php echo (int)$order_total; ?> р.";
						  sms = sms + "\n";
						  sms = sms + "Доставка 01.01.2017 с 12:00-15:00";
					      $('#sms_send_btn').removeAttr('disabled');
					  break;
					  
				  case "1":
					  	  sms = sms + "Ваш заказ № <?php echo $order_id; ?> отправлен.";
						  sms = sms + "\n";
						  sms = sms + "Номер Вашего отправления: 0000000000";
						  sms = sms + "\n";
						  sms = sms + "Служба доставки: Почта России";
					  $('#sms_send_btn').removeAttr('disabled');
					  break;
					  
				  case "2":
					  sms = sms + "Ваш заказ № <?php echo $order_id; ?> сформирован.";
					  sms = sms + "\n";
					  sms = sms + "Вы можете забрать заказ в офисе компании пн-чт: 9:00-18:00, пт:9:00-17:00";
					  $('#sms_send_btn').removeAttr('disabled');
				  break;
					  
				 case "3":
					 // sms = '';
					  //$('#sms_send_btn').addAttr('disabled');
				  break;				   }
...
$.ajax({
		url: '<?php echo $catalog; ?>index.php?route=api/order/sms&token=' + token + '&store_id=<?php echo $store_id; ?>&order_id=<?php echo $order_id; ?>',
		type: 'post',
		dataType: 'json',
		data: 'text=' + encodeURIComponent($('#sms_text').val()) + '&phone=' + encodeURIComponent($('#sms_phone').val()),
		beforeSend: function() {
			$('#sms_send_btn').button('loading');
		},
		complete: function() {
			$('#sms_send_btn').button('reset');
		},
		success: function(json) {
			$('.alert').remove();

			if (json['error']) {
				$('#sms_klient').before('<div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i> ' + json['error'] + ' <button type="button" class="close" data-dismiss="alert">&times;</button></div>');
			}

			if (json['success']) {
				//$('#sms_klient').load('index.php?route=sale/order/history&token=<?php echo $token; ?>&order_id=<?php echo $order_id; ?>');

				$('#sms_klient').before('<div class="alert alert-success"><i class="fa fa-check-circle"></i> ' + json['success'] + ' <button type="button" class="close" data-dismiss="alert">&times;</button></div>');
	   ...
	   $('#sms_btn').click(function(){
			  $.ajax({
				url: '<?php echo $catalog; ?>index.php?route=api/order/CheckSmsBalance&token=' + token + '&store_id=<?php echo $store_id; ?>',
				type: 'post',
				dataType: 'json',
				success: function(json) {
					
					if (parseInt(json['success']) < 30) {
						$('#sms_balance').css('color','red');
					} else {
						$('#sms_balance').css('color','green');
					}
					
					if (json['success']) {
						$('#sms_balance').html('Баланс: ' + json['success']);
